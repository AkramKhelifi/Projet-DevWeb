...
class RepoUser extends Repo{
    ...
    function verifierSuivie($nom) {
        // Préparer la requête SQL pour vérifier l'existence du suivi
        $sql = "SELECT COUNT(*) FROM suivi WHERE statut_suivi = :nom";
        $sth = $this->link->prepare($sql);
        $sth->bindParam(':nom', $nom, PDO::PARAM_STR); // Corrigé ici
        $sth->execute();
        
        // Récupérer le résultat
        $count = $sth->fetchColumn();
    
        // Retourner true si le suivi existe déjà, false sinon
        return $count > 0;
    }
