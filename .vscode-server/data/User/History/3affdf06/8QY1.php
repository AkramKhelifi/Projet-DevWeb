...
class RepoUser extends Repo{
    ...
    public function getInterventionsParIntervenant($idIntervenant) {
        ...
        $sql = "SELECT i.*, s.statut_suivi, u.deg_urgence FROM intervention i 
                INNER JOIN intervention_interven
                WHERE id_standardiste = :idStandardiste AND s.statut_suivi != 'CLOTURER'";
    
        $sth = $this->link->prepare($sql);
        $sth->bindParam(':idStandarditse', $idStandardiste, \PDO::PARAM_INT);
        $sth->execute();
