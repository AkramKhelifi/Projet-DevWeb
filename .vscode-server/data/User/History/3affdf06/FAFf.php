...
class RepoUser extends Repo{
    ...
    public function declencherIntervention($id_int, $idu, $id_urgence) {
        $sql = "INSERT INTO intervention (id_client, id_standardiste, id_urgence, nom_intervention, details_intervention, date_cre_int) 
                SELECT :idu, id_standardiste, :id_urgence, nom_intervention, details_intervention, CURRENT_TIMESTAMP
                FROM intervention
    ...
    public function getInterventionsPourStandardiste($idStandardiste) {
        $sql = "SELECT i.*, s.statut_suivi, u.deg_urgence FROM intervention i 
                INNER JOIN suivi s ON i.id_suivi = s.id_suivi
                INNER JOIN urgence u ON i.id_urgence = u.id_urgence
                WHERE i.id_standardiste = :idStandardiste AND s.statut_suivi != 'CLOTURER'";
    
        $sth = $this->link->prepare($sql);
        $sth->bindParam(':idStandardiste', $idStandardiste, \PDO::PARAM_INT);
        $sth->execute();
    ...
    public function getInterventionsdetails($idstd){
        $sql = "SELECT i.*, s.statut_suivi, u.deg_urgence FROM intervention i 
                INNER JOIN suivi s ON i.id_suivi = s.id_suivi
                INNER JOIN urgence u ON i.id_urgence = u.id_urgence
                WHERE (i.id_standardiste != :idstd OR i.id_standardiste IS NULL) AND s.statut_suivi != 'CLOTURER'";
    
        $sth = $this->link->prepare($sql);
        $sth->bindParam(':idstd', $idstd, \PDO::PARAM_INT);
        $sth->execute();
    ...
    public function insertChatMessage($userId, $message)
{
...
{
    $sql = 'SELECT cm.*, u.nom, u.prenom FROM chat_messages cm INNER JOIN users u ON cm.user_id = u.idu ORDER BY cm.created_at DESC';
    $sth = $this->link->prepare($sql);
    $sth->execute();
    return $sth->fetchAll(\PDO::FETCH_ASSOC);
}
