...
class RepoUser extends Repo{
    ...
    public function interventionDejaDeclenchee($id_int, $idu) {
        // La requête SQL pour vérifier l'existence d'une intervention spécifique déclenchée par un utilisateur donné
        $sql = "SELECT COUNT(*) FROM intervention WHERE id_intervention = :id_int AND id_client = :idu";
        
        // Préparer la requête SQL
        $sth = $this->link->prepare($sql);
        
        // Exécuter la requête avec les paramètres sous forme de tableau associatif
        $sth->execute([
            ':id_int' => $id_int,
            ':idu' => $idu
        ]);
        ...
        // Si le compteur est supérieur à 0, cela signifie que l'intervention a déjà été déclenchée
        return $count > 0;
    }
}