...
class RepoUser extends Repo{
    ...
    public function getInterventionsPourStandardiste($idStandardiste) {
        // Spécifiez les champs que vous voulez récupérer, par exemple i.*, s.statut_suivi, u.deg_urgence
        $sql = "SELECT i.*, s.statut_suivi, u.deg_urgence FROM intervention i 
                INNER JOIN suivi s ON i.id_suivi = s.id_suivi
                INNER JOIN urgence u ON i.id_urgence = u.id_urgence
        ...
        $sth = $this->link->prepare($sql);
        // Corrigez la faute de frappe ici : 'idStandarditse' -> 'idStandardiste'
