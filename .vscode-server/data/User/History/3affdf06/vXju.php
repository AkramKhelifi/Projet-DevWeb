...
class RepoUser extends Repo{
    ...
    public function getInterventionbyidu($idu) {
        // La requête SQL inclut maintenant une jointure avec la table suivi
        // pour récupérer le statut_suivi en plus des informations de l'intervention
        $sql = "SELECT intervention.*, suivi.statut_suivi 
                FROM intervention 
    ...
    public function declencherIntervention($id_int, $idu) {
        $sql = "INSERT INTO intervention (id_client, nom_intervention, details_intervention, date_cre_int) 
                SELECT :idu, nom_intervention, details_intervention, CURRENT_TIMESTAMP
                FROM intervention
