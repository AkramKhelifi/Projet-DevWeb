...
public function getInterventionbyidu($idu) {
    // La requête est mise à jour pour inclure une jointure avec la table urgence
    // pour récupérer deg_urgence en plus des informations de l'intervention
    $sql = "SELECT intervention.*, urgence.deg_urgence 
            FROM intervention 
            LEFT JOIN urgence ON intervention.id_urgence = urgence.id_urgence
            WHERE id_client = :idu OR id_admin = :idu OR id_standardiste = :idu OR id_intervenant = :idu";
