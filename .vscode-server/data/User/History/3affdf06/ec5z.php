...
class RepoUser extends Repo{
    ...
    public function getInterventionsParIntervenant($idIntervenant) {
        // Requête pour récupérer les interventions, leur statut de suivi et le degré d'urgence, associées à l'idIntervenant
        ...
        // Récupération des interventions, leur statut de suivi et degré d'urgence sous forme d'un tableau d'objets ou d'associatif
        return $sth->fetchAll(\PDO::FETCH_ASSOC);
    }


    public function getInterventionbyid2($id) {
        // Requête pour récupérer l'intervention et son statut de suivi
        ...
        // Récupération de l'intervention avec son statut de suivi sous forme d'un tableau associatif
        return $sth->fetch(\PDO::FETCH_ASSOC);
    }
    ...
    public function getSuivi() {
        ...
        // Récupération des statuts sous forme d'un tableau de chaînes
        return $sth->fetchAll(\PDO::FETCH_COLUMN, 0);
    }

    public function updateIntervention2($idu, $statut_suivi) {
        $sql = "UPDATE intervention SET id_suivi = (SELECT id_suivi FROM suivi WHERE statut_suivi = :statut_suivi) WHERE idu = :idu";
        $sth = $this->link->prepare($sql);
        $sth->bindParam(':role_name', $role_name, \PDO::PARAM_STR); 
        $sth->bindParam(':idu', $idu, \PDO::PARAM_INT);
        $sth->execute();
